...
from config.preprocessor import build_preprocessor
...
def evaluate_model(model, X_test, y_test, threshold: float, title: str,
                   file_path: str, cv=None):
    ...
    # Cross-validation per media e deviazione standard
    accuracies, precisions, recalls, f1s, aucs_list = [], [], [], [], []

    for train_idx, val_idx in cv.split(X_test, y_test):
        X_va = X_test.iloc[val_idx]
        y_va = y_test.iloc[val_idx]

        y_pred_fold, y_proba_fold = predict_with_threshold(
            model, X_va, threshold=threshold
        )

        accuracies.append(accuracy_score(y_va, y_pred_fold))
        precisions.append(precision_score(y_va, y_pred_fold, zero_division=0))
        recalls.append(recall_score(y_va, y_pred_fold, zero_division=0))
        f1s.append(f1_score(y_va, y_pred_fold, zero_division=0))
        aucs_list.append(roc_auc_score(y_va, y_proba_fold))
...
def plot_mean_roc_cv(estimator, X, y, title, out_path, n_splits=10, random_state=42):
    ...
    ax.plot(
        mean_fpr, mean_tpr, linewidth=2,
        label=f"Mean ROC (AUC = {mean_auc:.3f} +/- {std_auc:.3f})"
    )
    ax.fill_between(
        mean_fpr, tpr_lower, tpr_upper,
        alpha=0.15, label="+/- 1 std (TPR)"
    )
...
def train_all_models(X_train_raw, y_train, X_test_raw, y_test,
                     random_state: int = 42):
    # --- Logistic Regression ---
    log_file_path = "docs/log_results.txt"
    log_est = LogisticRegression(
        max_iter=3000, solver="lbfgs", random_state=random_state
    )
    log_grid = {
    ...
    log_model, log_mean, log_std, cv_log = _gridsearch_model(
        "Logistic Regression", log_est, log_grid,
        X_train_raw, y_train,
        file_path=log_file_path, random_state=random_state
    )
    evaluate_model(
        log_model, X_test_raw, y_test,
        threshold=0.5, title="Logistic Regression",
        file_path=log_file_path, cv=cv_log
    )
    ...
    dt_model, dt_mean, dt_std, cv_dt = _gridsearch_model(
        ...
        X_train_raw, y_train,
        file_path=dt_file_path, random_state=random_state
    )
    evaluate_model(
        dt_model, X_test_raw, y_test,
        threshold=0.5, title="Decision Tree",
        file_path=dt_file_path, cv=cv_dt
    )
    ...
    rf_model, rf_mean, rf_std, cv_rf = _gridsearch_model(
        ...
        X_train_raw, y_train,
        file_path=rf_file_path, random_state=random_state
    )
    evaluate_model(
        rf_model, X_test_raw, y_test,
        threshold=0.5,
        title="Random Forest ",
        file_path=rf_file_path, cv=cv_rf
    )
    ...
    knn_model, knn_mean, knn_std, cv_knn = _gridsearch_model(
        "KNN", knn_est, knn_grid,
        X_train_raw, y_train,
        file_path=knn_file_path, random_state=random_state
    )
    evaluate_model(
        knn_model, X_test_raw, y_test,
        threshold=0.5, title="KNN",
        file_path=knn_file_path, cv=cv_knn
    )
    ...
    mlp_model, mlp_mean, mlp_std, cv_mlp = _gridsearch_model(
        "MLP", mlp_est, mlp_params,
        X_train_raw, y_train,
        file_path=mlp_file_path, random_state=random_state,
        use_randomized=True, n_iter=8,
    )
    evaluate_model(
        mlp_model, X_test_raw, y_test,
        threshold=0.5, title="MLP",
        file_path=mlp_file_path, cv=cv_mlp
    )

    # --- Curve ROC ---
    print("\n>>> Generazione curve ROC per ogni modello...")
    plot_mean_roc_cv(
        log_model, X_train_raw, y_train,
        "LogReg - ROC (10-fold CV)", "docs/roc_logreg_cv.png"
    )
    plot_mean_roc_cv(
        dt_model, X_train_raw, y_train,
        "DT - ROC (10-fold CV)", "docs/roc_dt_cv.png"
    )
    plot_mean_roc_cv(
        rf_model, X_train_raw, y_train,
        "RF - ROC (10-fold CV)", "docs/roc_rf_cv.png"
    )
    plot_mean_roc_cv(
        knn_model, X_train_raw, y_train,
        "KNN - ROC (10-fold CV)", "docs/roc_knn_cv.png"
    )
    plot_mean_roc_cv(
        mlp_model, X_train_raw, y_train,
        "MLP - ROC (10-fold CV)", "docs/roc_mlp_cv.png"
    )
